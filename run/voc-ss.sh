...
exp --method COS --name ${name} --epochs 30 --lr 0.01 --batch_size 12 --val_interval 10 --contrast_loss 0.1 $4
...
for ns in 5; do  # shot 1/2/5 images
  for is in 0 1 2; do  # image samples
    inc_par="--ishot ${is} --input_mix novel --val_interval 100 --ckpt_interval 5"
      ...
      exp --method PIFS --name ${name} --iter ${iter} --lr ${lr} ${gen_par} ${inc_par} --step 1 --nshot ${ns} --step_ckpt ${path}/${name}_0.pth
  done
done
